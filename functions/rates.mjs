...
// Abort a fetch that takes too long so one slow/hanging upstream (e.g. Stooq, or a sluggish
// treasury.gov response) can't run the whole function into Netlify's ~10s wall and return empty.
...
async function fetchMonthEntries(yyyymmStr) {
  ...
  const res = await fetchWithTimeout(url, { headers: { "User-Agent": "Mozilla/5.0 (caf-rates-desk)" } }, 7000);
...
async function fetchForeignOne({ name, symbol }) {
  ...
  const res = await fetchWithTimeout(url, { headers: { "User-Agent": "Mozilla/5.0 (caf-rates-desk)" } }, 4500);
